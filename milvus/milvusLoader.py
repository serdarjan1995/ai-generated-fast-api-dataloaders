...
app = FastAPI(title='Milvus Loader')
...
def load_data_endpoint(query: QueryModel, token: str = Depends(get_api_key)):
    reader = MilvusReader(host='localhost', port=19530, user='<user>', password='<password>', use_secure=False)
    documents = reader.load_data(query_vector=query.query_vector, collection_name=query.collection_name, limit=query.limit)
    return documents
