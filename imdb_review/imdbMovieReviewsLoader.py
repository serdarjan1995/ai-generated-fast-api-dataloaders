...
class IMDBReviews(BaseReader):
    def __init__(
        ...
        Args:
            movie_name_year (str): movie name along with year
...
async def load_movie_reviews(
    movie_name_year: str = Query(
        ...,
        description='The name of the movie or series and year, e.g., "The Social Network 2010"',
    ),
    webdriver_engine: str = Query(
        "google",
        enum=["google", "edge", "firefox"],
        description="The webdriver engine to use",
    ),
    generate_csv: Optional[bool] = Query(
        False, description="Whether to generate a CSV file or not"
    ),
    multithreading: Optional[bool] = Query(
        False, description="Whether to use multithreading or not"
    ),
    max_workers: int = Query(
        0, description="The number of workers to use if multithreading is enabled"
    ),
    reviews_folder: str = Query(
        "movie_reviews", description="The folder path to save the reviews"
    ),
):
    try:
