...
class MemosReader:
    ...
    def load_data(self, params: Optional[Dict] = None):
        ...
        documents = [Document(text=memo['content'], extra_info={'creator': memo['creator'], 'resource_list': memo['resourceList'], id: memo['id'],}) for memo in data['data']]
        return documents
...
async def load_data(host: str = Query(..., description='Host where memos is deployed'), params: Optional[Dict] = Query(None, description='Filtering parameters')):
